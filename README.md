...
``` bash 
python pipedream.py trigger-branch {new_branch_name}
```
...
In order to deploy demo infrastructure navigate to ``/infrastructure`` and replace all instances of ``{{aws-account}}`` with your account number in ``pipedream/infrastructure/jobs.json`` file. Then run the following command: 
    ``` bash
...
4.	Create a new branch of a node of your choice using the following command:
    ``` bash 
    python pipedream.py create-branch node_name new_branch
    ```
...
5.	Deploy your branch using the following command:
    ``` bash
    python pipedream.py deploy-branch new_branch
    ```
...
9.	Use the ``destroy-branch`` command to destroy a deployed branch:
    ``` bash
    python pipedream.py destroy-branch new_branch
    ```

10.	Use the ``delete-branch`` command to delete a local branch:
    ``` bash
    python pipedream.py delete-branch new_branch
    ```
